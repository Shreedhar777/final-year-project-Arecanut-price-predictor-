...
def main():
    ...
    price=list(result['Sales'])
    
    date=list(result['Date'])
    
    print("list is",price,date)
    ...
    labels = ['Linear', 'KNeighbors', 'RandomForest','Lasso','DecisionTree']
    plt1.xticks(range(len(data)), labels)
    plt1.xlabel('ML Algorithm')
    plt1.ylabel('prediction score')
    plt1.title('Arecanaut price prediction')
    plt1.bar(range(len(data)), data) 
    plt1.savefig('final.jpg')
    plt1.show()
    return price,date
